     ...
     // Create a new info window
         var infoWindow = new google.maps.InfoWindow({
          content: '<b>Case No: #145</b>' +
                   ...
                   '<br><br><form>'+
                   '<button type="submit" onclick="submitForm()">Verify</button>' +
                   '<button type="submit" onclick="submitForm()">Severity</button>' +
                   '<button type="submit" onclick="submitForm()">Assign</button>'+
                   '<button type="submit" onclick="submitForm()">Status</button>' +
                   '</form>'
        });
